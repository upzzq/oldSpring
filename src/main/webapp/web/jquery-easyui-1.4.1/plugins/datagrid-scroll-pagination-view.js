/**
 * 重写view，实现单页显示千条以上的数据，
 * @param
 * scrollOption{
 * 	isRemote:true,//异步分页 默认为false
 * 	pageSize:20,//单页滚动大小，区别于datagrid的pageSize
 *  idField:issueId,//主键fieldName，记录check的内容
 *  checkedRows:[],返回使用checkbox当地勾选的所有数据
 * }
 * 注：此view重写了datagrid的以下事件，如需监听，
 * 请调用父方法，以免导致当前功能不可用
 * Override events:
 * onBeforeLoad
 * onSelect
 ...
 * onUncheckAll
 */
var scrollAndPaginationView = $.extend({}, $.fn.datagrid.defaults.view, {
	render: function(target, container, frozen){
		...
		for(var i=0; i<rows.length; i++) {
			...
			var rowData = rows[i] ;
			var isChecked = false ;
			if(opts.scrollOption && opts.scrollOption.idField){
				var idValue = rowData[opts.scrollOption.idField];
				if(opts.scrollOption.checkedRows 
						&& $.inArray(idValue+"",opts.scrollOption.checkedRows) != -1){
					isChecked = true ;
				}
			}
			if(isChecked){
				classValue += " datagrid-row-selected " ;
			}
...
//			var style = styleValue ? 'style="' + styleValue + '"' : '';
			...
			table.push(this.renderRow.call(this, target, fields, frozen, index, rowData, isChecked));
			table.push('</tr>');
		...
		$(".scroll-grid-check").change(function(){
			var idValue = $(this).attr("idValue")
	      	if($(this).attr("checked") == "checked"){
	      		_this.addCheckRow(opts,idValue);
	      	}else{
	      		_this.removeCheckRow(opts,idValue);
	      	}
	    });
	},
	addCheckRow:function(opts,idValue){
		if(opts.scrollOption){
			if(!opts.scrollOption.checkedRows){
				opts.scrollOption.checkedRows = [] ;
			}
			var index = jQuery.inArray(idValue+"",opts.scrollOption.checkedRows);
			if(index == -1)
				opts.scrollOption.checkedRows.push(idValue+"");
		}
	},
	removeCheckRow:function(opts,idValue){
		if(opts.scrollOption && opts.scrollOption.checkedRows){
			var index = jQuery.inArray(idValue+"",opts.scrollOption.checkedRows);
			if(index != -1)
				opts.scrollOption.checkedRows.splice(index,1);
		}
	},
	renderRow: function(target, fields, frozen, rowIndex, rowData ,isChecked){
		var opts = $.data(target, 'datagrid').options;
		var pageSize = opts.pageSize;//此处为记录行号
		var cc = [];
		if (frozen && opts.rownumbers){
			var rownumber = rowIndex + 1;
			if (opts.pagination){
				rownumber += (opts.pageNumber-1)*pageSize;
			}
			cc.push('<td class="datagrid-td-rownumber"><div class="datagrid-cell-rownumber">'+rownumber+'</div></td>');
		}
		var idValue = null ;
		if(opts.scrollOption && opts.scrollOption.idField){
			idValue = rowData[opts.scrollOption.idField];
		}
		for(var i=0; i<fields.length; i++){
			...
			if (col){
				...
				if (col.checkbox){
					...
					cc.push('<input type="checkbox" class="scroll-grid-check" '+checked+' name="' + field + '" idValue="'+idValue+'" value="' + (value!=undefined ? value : '') + '">');
				} else if (col.expander) {
	...
	onBeforeRender: function(target){
		...
		function scrolling(){
			if (dc.body2.is(':empty')){
				...
				var pageSize = view.getPageSize(opts);
				if (top > dc.body2.height() || bottom < 0){
					...
					var page = Math.floor(this.index/pageSize)+2;
					if (this.r2.length){
						page++;
					}
					this.getRows.call(this, target, page, function(rows){
	...
	getRows: function(target, page, callback){
		var state = $.data(target, 'datagrid');
		var opts = state.options;
		var pageSize = this.getPageSize(opts);
		var index = (page-1)*pageSize;
		var rows = state.data.rows.slice(index, index+pageSize);
		if (rows.length){
			callback.call(this, rows);
			
		} else {
			if(!opts.scrollOption || !opts.scrollOption.isRemote)
				return ;
...
//				opts.onLoadSuccess.call(target, data);
	...
	getPageSize:function(opts){
		if(opts.scrollOption && opts.scrollOption.pageSize)
			return opts.scrollOption.pageSize ; 
		return opts.pageSize ;
	}
});
$.fn.datagrid.defaults.onBeforeLoad = function(a){
	var opts = $(this).datagrid("options");
	if(opts.scrollOption){
		opts.scrollOption.checkedRows = new Array() ;
	}
};
$.fn.datagrid.defaults.onSelect = function(i,d){
	var opts = $(this).datagrid("options");
	if(opts.scrollOption 
			&& opts.scrollOption.idField 
			&& opts.checkOnSelect){
		var idValue = d[opts.scrollOption.idField];
		opts.view.addCheckRow(opts,idValue);
	}
};
$.fn.datagrid.defaults.onUnselect = function(i,d){
	var opts = $(this).datagrid("options");
	if(opts.scrollOption 
			&& opts.scrollOption.idField 
			&& opts.checkOnSelect){
		var idValue = d[opts.scrollOption.idField];
		opts.view.removeCheckRow(opts,idValue);
		opts.checkedAll = false ;
	}
};
...
$.fn.datagrid.defaults.onCheckAll = function(rows){
	...
	if(opts.scrollOption 
		...
		opts.checkedAll = true ;
		$.each(rows,function(i,r){
			var idVal = r[opts.scrollOption.idField];
			opts.scrollOption.checkedRows.push(idVal+"");
		});
	}
};
$.fn.datagrid.defaults.onUncheckAll = function(rows){
	var opts = $(this).datagrid("options");
	opts.checkedAll = false ;
	if(opts.scrollOption 
			&& opts.scrollOption.idField 
			&& opts.scrollOption.checkedRows){
		opts.scrollOption.checkedRows = new Array() ;
		$(".scroll-grid-check").removeAttr("checked");
	}
};
$.extend($.fn.datagrid.methods, {
	clearChecked:function(jq){
		try{
			...
			if(opts.scrollOption 
				...
				opts.scrollOption.checkedRows = new Array() ;
				$(".scroll-grid-check").removeAttr("checked");
				$('#org_sel_center').find(".datagrid-header-check").find("input[type=checkbox]").removeAttr("checked");
			}
		}catch(e){}
	},
	...
	addCheckRow:function(jq,idValue){
		var options = $(jq).datagrid('options');
		options.view.addCheckRow(options,idValue);
	},
	removeCheckRow:function(jq,idValue){
		var options = $(jq).datagrid('options');
		options.view.removeCheckRow(options,idValue);
	},
	getCheckedIdField: function(jq){
		var opts = $(jq).datagrid('options');
		if(opts.scrollOption && opts.scrollOption.checkedRows){
			return opts.scrollOption.checkedRows;
		}
		return $.data(jq[0],"datagrid").checkedRows;
	},
	getChecked: function(jq){
		var opts = $(jq).datagrid('options');
		var tData = [] ;
		if(opts.checkedAll){
			return $(jq).datagrid('getRows');
		}
		if(opts.scrollOption 
				&& opts.scrollOption.checkedRows
				&& opts.scrollOption.checkedRows.length > 0){
			var checkedRows = opts.scrollOption.checkedRows;
			if(opts.idField){
				return checkedRows ;
			}
			...
			$.each(rows,function(i,r){
				var idVal = r[opts.scrollOption.idField];
				if($.inArray(idVal+"",checkedRows) != -1){
					tData.push(r);
				}
			});
			return tData ;
		...
		var _f2=[];
		opts.finder.getTr(jq[0],"","checked",2).each(function(){
		_f2.push(opts.finder.getRow(jq[0],$(this)));
		});
		return _f2;
		}
	}
